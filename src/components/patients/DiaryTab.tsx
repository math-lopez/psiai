...
import { 
  ...
  Smile, MessageSquare, BrainCircuit, Rocket, Loader2, Edit2
} from "lucide-react";
...
export const DiaryTab = ({ patientId }: { patientId: string }) => {
  ...
  const handleSaveLog = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (editingLog) {
        // Lógica de atualização (apenas autor)
        await diaryService.updateLog(editingLog.id, logData);
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 animate-in fade-in duration-500">
      <div className="lg:col-span-2 space-y-8">
        <div className="flex items-center justify-between px-2">
          ...
          <div className="flex items-center gap-2">
            <Select value={filter} onValueChange={(v: any) => setFilter(v)}>
              <SelectTrigger className="h-10 w-[180px] rounded-2xl border-none bg-white dark:bg-slate-900 shadow-sm font-bold text-xs uppercase tracking-widest text-slate-500">
            ...
            <Button onClick={() => { setEditingLog(null); setIsLogModalOpen(true); }} className="bg-indigo-600 h-10 px-6 rounded-2xl font-black shadow-lg shadow-indigo-100 gap-2">
        ...
        <div className="space-y-6">
          {filteredLogs.length > 0 ? filteredLogs.map((log) => {
            ...
            const isAuthor = log.psychologist_id === user?.id && log.created_by === 'psychologist';
            
            return (
              <Card key={log.id} className="border-none shadow-sm rounded-[32px] bg-white dark:bg-slate-900 overflow-hidden group hover:shadow-md transition-all">
                <CardContent className="p-8 space-y-4">
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h4 className="font-bold text-slate-900 dark:text-white">{log.title || typeInfo.label}</h4>
                    ...
                    {isAuthor && (
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="rounded-xl">
                          <DropdownMenuItem className="gap-2" onClick={() => { setEditingLog(log); setIsLogModalOpen(true); }}>
                  ...
                  <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed whitespace-pre-wrap italic">"{log.content}"</p>
                  <div className="flex items-center gap-4 pt-4 border-t border-slate-50 dark:border-slate-800">
                    <Badge variant="outline" className="border-slate-100 dark:border-slate-800 text-slate-400 text-[8px] font-black uppercase tracking-widest px-2.5">
            ...
            <div className="text-center py-20 bg-slate-50/50 dark:bg-slate-800/50 rounded-[40px] border-2 border-dashed border-slate-100 dark:border-slate-800">
               <Book className="h-10 w-10 text-slate-200 dark:text-slate-700 mx-auto mb-3" />
      ...
      <div className="space-y-8">
        <div className="flex items-center justify-between px-2">
           ...
           <Button variant="ghost" size="icon" onClick={() => setIsPromptModalOpen(true)} className="h-8 w-8 rounded-xl bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600">
        ...
        <div className="space-y-4">
          {prompts.filter(p => p.status === 'active').length > 0 ? prompts.filter(p => p.status === 'active').map((prompt) => (
            <Card key={prompt.id} className="border-none shadow-sm rounded-[28px] bg-white dark:bg-slate-900 overflow-hidden group">
              <CardContent className="p-6 space-y-3">
                <div className="flex items-start justify-between gap-3">
                   <h4 className="font-bold text-sm text-slate-900 dark:text-white leading-tight">{prompt.title}</h4>
                   <button onClick={() => togglePromptStatus(prompt)} className="h-5 w-5 rounded-full border-2 border-slate-200 dark:border-slate-700 group-hover:border-indigo-400 transition-colors flex items-center justify-center shrink-0">
            ...
            <div className="text-center py-10 bg-white dark:bg-slate-900 rounded-[32px] border border-slate-50 dark:border-slate-800">
      ...
      <Dialog open={isLogModalOpen} onOpenChange={setIsLogModalOpen}>
        <DialogContent className="rounded-[32px] sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle className="text-xl font-black">{editingLog ? "Editar Registro" : "Registrar Acompanhamento"}</DialogTitle>
            <DialogDescription>Anotações sobre a evolução do paciente.</DialogDescription>
          </DialogHeader>
          <form onSubmit={handleSaveLog} className="space-y-4 pt-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Select name="log_type" defaultValue={editingLog?.log_type || "general"} required>
              ...
              <div className="space-y-2">
                ...
                <Select name="visibility" defaultValue={editingLog?.visibility || "private_to_psychologist"} required>
                  <SelectTrigger className="rounded-xl h-12">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent className="rounded-xl">
            ...
            <div className="space-y-2">
              <Label htmlFor="title" className="text-xs font-black uppercase text-slate-400">Título</Label>
              <Input id="title" name="title" defaultValue={editingLog?.title || ''} className="rounded-xl h-12" placeholder="Título opcional..." />
            </div>
            <div className="space-y-2">
              <Label htmlFor="content" className="text-xs font-black uppercase text-slate-400">Conteúdo</Label>
              <Textarea id="content" name="content" defaultValue={editingLog?.content} required className="rounded-xl min-h-[150px]" placeholder="Descreva os fatos..." />
            </div>
        ...
        <DialogContent className="rounded-[32px] sm:max-w-[425px]">
          ...
          <form onSubmit={handleSavePrompt} className="space-y-4 pt-4">
            <div className="space-y-2">
              ...
              <Input id="p-title" name="title" required className="rounded-xl h-12" placeholder="Ex: Registrar sentimentos..." />
            </div>
            <div className="space-y-2">
              <Label htmlFor="due_date" className="text-xs font-black uppercase text-slate-400">Data Alvo</Label>
