...
import { Users, Calendar, Clock, CheckCircle2, Plus, ArrowRight, Loader2, Sparkles } from "lucide-react";
...
const StatCard = ({ title, value, icon: Icon, gradient, index }: any) => (
          ...
          <CardContent className="pt-6">
            <div className="space-y-3">
              {sessions.length > 0 ? sessions.slice(0, 4).map((session) => (
                <div key={session.id} className="flex items-center justify-between p-4 rounded-2xl border border-transparent hover:border-slate-100 hover:bg-slate-50 transition-all group">
                ...
                <div className="text-center py-10">
                   ...
                   <p className="text-xs text-slate-400 font-medium">Nenhuma sessão recente.</p>
                </div>
              )}
            </div>
          </CardContent>
