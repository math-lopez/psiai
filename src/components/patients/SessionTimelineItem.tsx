...
export const SessionTimelineItem = ({ session }: SessionTimelineItemProps) => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'cancelled':
        return <Badge className="bg-red-50 text-red-600 hover:bg-red-50 border-none gap-1 font-bold text-[10px]"><XCircle className="h-3 w-3" /> Cancelada</Badge>;
      case 'error':
  ...
  const isCancelled = session.processing_status === 'cancelled';
  ...
  return (
    <div className="relative pl-8 pb-10 group last:pb-0">
      ...
      <div className={cn(
        ...
        session.processing_status === 'cancelled' ? "bg-red-400" : "bg-slate-300"
      )}>
        <div className="h-1.5 w-1.5 rounded-full bg-white" />
      </div>

      <div className={cn(
        "bg-white rounded-[32px] border border-slate-100 p-6 shadow-sm hover:shadow-md transition-all",
        ...
        {isCancelled && (
          <p className="text-xs font-bold text-slate-400 italic mt-2">Atendimento cancelado.</p>
        )}
      </div>
    </div>
  );
